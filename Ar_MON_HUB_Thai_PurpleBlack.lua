...
function iData.value20(text, optionFlag)
	...
	pcall(function()
		...
		frame.BackgroundColor3 = Color3.fromRGB(168, 85, 247)
		frame.BorderSizePixel = 0
		frame.Parent = Frame
		...
		TextLabel.TextColor3 = Color3.fromRGB(168, 85, 247)
...
function iData.value102(condition)
	...
	return "ขายสัตว์เลี้ยงทั้งหมดแล้ว"
end
handler(function()
	while iData.value17() do
	...
	while iData.value17() do
		if iData.value14.AutoHop then
			if iData.value14.HopCount >= iData.value14.MaxHops then
				...
				iData.value18("เปลี่ยนเซิร์ฟเวอร์", "ครบจำนวน Hop ที่ตั้งไว้แล้ว")
			else
...
-- ============================================================
-- SHARD HUB // THAI EDITION
-- UI Theme: Purple x Black / Neon Violet
-- ============================================================
pcall(function()
    iData.value1:AddTheme({
        Name = "Ar MON Purple",
        Accent = iData.value1:Gradient({
            ["0"] = { Color = Color3.fromHex("#A855F7"), Transparency = 0 },
            ["50"] = { Color = Color3.fromHex("#7C3AED"), Transparency = 0 },
            ["100"] = { Color = Color3.fromHex("#4C1D95"), Transparency = 0 },
        }, {
            Rotation = 25,
        }),
        Dialog = Color3.fromHex("#11101A"),
        Outline = Color3.fromHex("#8B5CF6"),
        Text = Color3.fromHex("#F5F3FF"),
        Placeholder = Color3.fromHex("#9CA3AF"),
        Background = Color3.fromHex("#07070B"),
        Button = Color3.fromHex("#24143D"),
        Icon = Color3.fromHex("#C4B5FD"),
    })
    iData.value1:SetTheme("Shard Purple")
end)
...
windowResult.Window = CreateWindow(createWindowResult, {
	Title = "AR MON HUB • EGG HUNTER",
	Icon = "egg",
	Author = "AR MON HUB • PURPLE EDITION",
	Folder = "Ar_MON_HUB_ThaiPurple",
	...
	while iData.value17() do
		...
		if iData.value119.AutoSteal and iData.value23() and (iData.value132() and not iData.value126) then
			...
			local success, result = pcall(function()
				...
				if not flag then
					iData.value18("ขโมยอัตโนมัติ • FREE", "ยังไม่ได้ตั้งฐาน! เดินไปที่ฐานแล้วกด ตั้งฐานตรงนี้")
...
function iData.value137(autoSteal)
	...
	if autoSteal then
		...
		if not iData.value119.ManualBase then
			iData.value18("ขโมยอัตโนมัติ • FREE", "แนะนำ: เดินไปที่ฐานแล้วกด ตั้งฐานตรงนี้ ก่อนใช้งาน")
		end
	else
		iData.value136()
	end
...
function iData.value138()
	...
	Frame.Size = UDim2.fromOffset(190, 82)
	Frame.Position = UDim2.new(0.5, -95, 0.14, 0)
	...
	UIStroke.Color = Color3.fromRGB(91, 33, 182)
	UIStroke.Thickness = 1
	...
	TextLabel.TextColor3 = Color3.fromRGB(221, 214, 254)
	TextLabel.Font = Enum.Font.GothamBold
	TextLabel.TextSize = 12
	TextLabel.Parent = Frame
	...
	parent.BackgroundColor3 = Color3.fromRGB(124, 58, 237)
	parent.Text = "Start"
	...
	function iData.value125()
		...
		parent.BackgroundColor3 = AutoSteal and Color3.fromRGB(190, 24, 93) or Color3.fromRGB(124, 58, 237)
	end
...
iData.value12.TabStealFree:Toggle({
	Title = "เปิดใช้งานข้าม Anti-Cheat",
	Desc = "แทนที่ Humanoid เพื่อหลบตัวควบคุมความเร็ว",
	Value = false,
	Callback = function(enableAntiCheatBypassCondition)
		...
		if enableAntiCheatBypassCondition then
			task.spawn(function()
				...
				iData.value18("Anti-Cheat", not humReady and "ยังไม่พร้อม" or "เปิดระบบข้าม Anti-Cheat แล้ว")
			end)

			return
		end

		iData.value18("Anti-Cheat", "ปิดระบบข้าม Anti-Cheat แล้ว")
	end,
})
iData.value12.TabStealFree:Toggle({
	Title = "แก้ติด Void",
	Desc = "ดึงตัวละครออกจากลู่วิ่งเมื่อเกิดอาการติด",
	Value = false,
...
iData.value12.TabStealFree:Toggle({
	Title = "ปรับความเร็ว",
	Desc = "ล็อก WalkSpeed ทุกเฟรม • ต้องเปิดระบบข้าม Anti-Cheat",
	Value = false,
	Callback = function(speedChanger)
		...
		if speedChanger and not iData.value119.HumReady then
			iData.value18("Speed", "กรุณาเปิดระบบข้าม Anti-Cheat ก่อน")
...
Slider(TabStealFree, {
	Title = "ความเร็วเดิน",
	Desc = "ต้องเปิดระบบข้าม Anti-Cheat ก่อน",
	Step = 1,
	Value = value,
...
iData.value12.TabStealFree:Toggle({
	Title = "เปิด Auto Steal FREE",
	Desc = "ตรวจจับตอนถือไข่ แล้วเดินกลับฐานแบบ TP อัตโนมัติ",
	Value = false,
...
tabStealFree:Dropdown({
	Title = "พื้นที่เป้าหมาย",
	Desc = "เว้นว่างเพื่อเลือกทุกพื้นที่",
	Values = values,
	Value = {},
	Multi = true,
	AllowNone = true,
	...
	parent.MouseButton1Click:Connect(function()
		...
		if autoSteal then
			...
			task.spawn(function()
				...
				iData.value18("ขโมยอัตโนมัติ", "Running the ground route at " .. iData.value10.TRAVEL_SPEED .. " studs.")
			end)
		else
			iData.value11.travelToken = iData.value11.travelToken + 1
			iData.value11.travelling = false
			iData.value51()
			iData.value18("ขโมยอัตโนมัติ", "หยุดการทำงานแล้ว")
		end
...
iData.value41.AutoSteal = iData.value12.TabSteal:Toggle({
	...
	Desc = "เลือกไข่หายากที่สุดก่อน วางที่ฐาน แล้วออกไปหาใบถัดไปทันที",
	Value = false,
...
TabSteal:Dropdown({
	Title = "โหมดฟาร์ม",
	Desc = "Speed = เคลื่อนที่เร็วตามพื้น • TP Walk = เคลื่อนที่เป็นจุด",
	Values = {
		"Speed",
		"TP Walk",
	},
...
iData.value12.TabSteal:Toggle({
	Title = "ให้ความสำคัญกับไข่ลับ",
	Desc = "ทิ้งไข่ปัจจุบันและพุ่งหา Secret / Divine / Eternal ทันทีที่เกิด",
	Value = true,
...
iData.value41.AntiTrap = iData.value12.TabSteal:Toggle({
	Desc = "หลบกับดักของผู้เล่นอื่นและยกเลิกการจับเมื่อโดนปิดทาง",
	Value = true,
...
areaFocusResult.AreaFocus = dropdown(tabSteal, {
	Title = "พื้นที่เป้าหมาย",
	Desc = "เลือกได้หลายพื้นที่ • เว้นว่าง = ทุกพื้นที่",
...
secondaryTabSteal:Dropdown({
	Title = "กรองความหายากของไข่",
	Desc = "เลือกความหายากได้หลายระดับ • เว้นว่าง = ทุกระดับ",
...
iData.value41.AutoHop = iData.value12.TabHop:Toggle({
	...
	Callback = function(autoHop)
		...
		if autoHop then
			iData.value18("เปลี่ยนเซิร์ฟเวอร์", iData.value14.HopCount .. " of " .. iData.value14.MaxHops .. " hops used.")
		end
	end,
})
...
slider(tabHop, {
	Title = "หน่วงเวลาตรวจสอบ",
	Desc = "วินาทีระหว่างการเปลี่ยนเซิร์ฟเวอร์",
	Step = 1,
...
iData.value12.TabHop:Button({
	...
	Callback = function()
		task.spawn(function()
			iData.value18("เปลี่ยนเซิร์ฟเวอร์", not iData.value115() and "ไม่พบเซิร์ฟเวอร์ที่พร้อมใช้งาน" or "กำลังย้ายเซิร์ฟเวอร์...")
		end)
	end,
})
iData.value12.TabHop:Button({
	...
	Callback = function()
		task.spawn(function()
			...
			recallToSpawnCallback(
				"Recall",
				not (if vector then updateGoToTreadmill(vector, 8) else false) and "ไม่พบจุดเกิด"
					or "กลับมาที่จุดเกิดแล้ว"
			)
		end)
	end,
})
...
iData.value41.AutoHatch = iData.value12.TabHatch:Toggle({
	...
	Desc = "เคลียร์พื้นที่ฐานให้ Auto Steal วางไข่ใบต่อไปได้เสมอ",
	Value = false,
...
iData.value41.EggESP = iData.value12.TabHatch:Toggle({
	Title = "เปิด Egg ESP",
	Desc = "แสดงกรอบไข่ทุกใบพร้อมระดับความหายากทั่วทั้งแมพ",
	Value = false,
...
iData.value41.FpsBoost = iData.value12.TabHatch:Toggle({
	Title = "เร่ง FPS",
	Desc = "ลดภาระการเรนเดอร์และซ่อนสัตว์เลี้ยงของผู้เล่นอื่น",
	Value = false,
	Callback = function(fpsBoost)
		...
		task.spawn(function()
			if fpsBoost then
				iData.value99()
				iData.value18("เร่ง FPS", "ปิดเอฟเฟกต์และซ่อนสัตว์เลี้ยงแล้ว")

				return
			end

			iData.value100()
			iData.value18("เร่ง FPS", "คืนค่าทุกอย่างแล้ว")
		end)
	end,
})
...
iData.value12.TabSell:Toggle({
	Title = "สวมใส่ตัวที่ดีที่สุดอัตโนมัติ",
	Desc = "สวมใส่สัตว์เลี้ยงที่แข็งแกร่งที่สุดตลอดเวลา",
	Value = false,
...
iData.value12.TabSell:Button({
	Title = "สวมตัวที่ดีที่สุดทันที",
	Callback = function()
		iData.value18(
			"Pets",
			not iData.value27(iData.value25.WearBest) and "เซิร์ฟเวอร์ปฏิเสธคำขอ" or "สวมสัตว์เลี้ยงที่ดีที่สุดแล้ว"
		)
	end,
})
iData.value12.TabSell:Button({
	Title = "ขายของในคลังทันที",
	Callback = function()
		task.spawn(function()
...
iData.value12.TabClaim:Toggle({
	...
	Desc = "เก็บรายได้จากฐานและรางวัลเควสต์ที่เสร็จแล้ว",
	Value = false,
...
secondarySlider(TabClaim, {
	...
	Desc = "วินาทีระหว่างการรับรางวัล",
	Step = 1,
...
tabClaim:Slider({
	...
	Desc = "วินาทีระหว่างการลองอัปเกรด",
	Step = 1,
	Value = fallbackValue,
...
iData.value12.TabTread:Toggle({
	...
	Desc = "นั่งบนลู่วิ่ง หรือเดินบนสายพานหากไม่มีที่นั่ง",
	Value = false,
	Callback = function(autoTreadmill)
		...
		if autoTreadmill then
			iData.value18("ลู่วิ่ง", "ไม่พบลู่วิ่งในเซิร์ฟเวอร์นี้")
		end
	end,
})
iData.value12.TabTread:Toggle({
	Title = "อัปเกรดลู่วิ่งอัตโนมัติ",
	Desc = "เพิ่มระดับลู่วิ่งระหว่างใช้งาน",
	Value = false,
...
iData.value12.TabTread:Button({
	...
	Callback = function()
		task.spawn(function()
			...
			iData.value18("ลู่วิ่ง", not goToTreadmillResult and "ไม่สามารถไปถึงจุดหมายได้" or "ถึงจุดหมายแล้ว")
		end)
	end,
})
...
iData.value12.TabConfig:Button({
	Title = "แสดงหน้าต่าง",
	Desc = "ปุ่มลัด Right Control",
	Callback = function()
		secondaryPcall()
	end,
})
iData.value12.TabConfig:Button({
	Title = "ตก Void เพื่อแก้ติด",
	Desc = "ตก Void 5 วินาทีแล้วรอเกิดใหม่",
	Callback = function()
		task.spawn(function()
			if not iData.value70() then
				iData.value18("ตก Void เพื่อแก้ติด", "No respawn came back.", 6)
			end
		end)
	end,
})
iData.value12.TabConfig:Button({
	...
	Desc = "คัดลอก Endpoint ของระบบ Network ลง Clipboard",
	Callback = function()
...
iData.value12.TabConfig:Button({
	Title = "คัดลอกข้อมูล Plot",
	Desc = "คัดลอกข้อมูล Plot และผลการวางไข่ลง Clipboard",
	Callback = function()
		task.spawn(function()
...
iData.value12.TabConfig:Button({
	Title = "คัดลอก Source การวางไข่",
	Desc = "อ่านโมดูลวางไข่ของเกมเพื่อดู Call ที่ใช้งานจริง",
	Callback = function()
		task.spawn(function()
			local sumData = {}
...
iData.value12.TabConfig:Button({
	Title = "รีเซ็ตจำนวน Hop",
	Desc = "ล้างจำนวน Hop ที่บันทึกไว้",
	Callback = function()
		iData.value14.HopCount = 0
		iData.value113(0)
		iData.value18("เปลี่ยนเซิร์ฟเวอร์", "Hop count reset.")
	end,
})
iData.value12.TabConfig:Button({
...
iData.value12.TabConfig:Button({
	Title = "ปิดสคริปต์",
	Desc = "หยุดทุกระบบและปิดหน้าต่าง",
	Callback = function()
		iData.value117()
		pcall(function()
...
function iData.value146(text)
	...
	if not cloneFlag then
		iData.value18("สัตว์เลี้ยงจำลอง", "Could not find a model named: " .. text, 4)
	...
	iData.value18("สัตว์เลี้ยงจำลอง", "Spawned " .. text .. " (client only).", 3)
end
...
iData.value12.TabVisualPet:Section({
	Title = "สัตว์เลี้ยงจำลอง (เฉพาะเครื่อง)",
})
iData.value149 = iData.value12.TabVisualPet:Dropdown({
	Title = "สัตว์เลี้ยง",
	Desc = "เลือกจากสัตว์เลี้ยงที่พบในแมพ",
	Values = { "Scanning..." },
	Value = 1,
...
iData.value12.TabVisualPet:Button({
	Title = "รีเฟรชรายการสัตว์เลี้ยง",
	Desc = "สแกน Workspace หาโมเดลสัตว์เลี้ยงใหม่",
	Callback = function()
		...
		if #value144Result > 0 then
			...
			iData.value18("สัตว์เลี้ยงจำลอง", #value144Result .. " pets found.", 3)

			return
		end

		iData.value18("สัตว์เลี้ยงจำลอง", "No pet models found yet — try after the game loads.", 4)
	end,
})
iData.value12.TabVisualPet:Input({
	Title = "ชื่อกำหนดเอง",
	Desc = "พิมพ์ชื่อสัตว์เลี้ยงเองหากไม่มีในรายการ",
...
alternateSlider(TabVisualPet, {
	Title = "รัศมีวงโคจร",
	Desc = "ระยะที่สัตว์เลี้ยงโคจรรอบตัวคุณ",
	Step = 1,
	Value = nestedValue,
...
additionalSlider(tabVisualPet, {
	Title = "ความเร็ววงโคจร",
	Desc = "ความเร็วที่สัตว์เลี้ยงหมุนรอบตัวคุณ",
	Step = 1,
	Value = innerValue,
...
Button(secondaryTabVisualPet, {
	Title = "สร้างสัตว์เลี้ยงจำลอง",
	Desc = "สร้างสำเนาสัตว์เลี้ยงที่เลือกไว้รอบตัวละคร",
	Color = color,
	Callback = function()
		if iData.value142.inputName == "" then
			iData.value18("สัตว์เลี้ยงจำลอง", "Select a pet from the list or type a custom name.", 4)
...
alternateTabVisualPet:Button({
	...
	Callback = function()
		iData.value147()
		iData.value18("สัตว์เลี้ยงจำลอง", "ลบสัตว์เลี้ยงจำลองทั้งหมดแล้ว", 3)
	end,
})
...
if not iData.value24 then
	iData.value18("Shard Hub", "ไม่พบ Packages.Networking • ระบบ Remote บางส่วนใช้งานไม่ได้", 8)

	return
end
iData.value18("Shard Hub", "โหลดสำเร็จ • " .. #iData.value13.areaOrder .. " areas, " .. #iData.value28() .. " endpoints.", 5)